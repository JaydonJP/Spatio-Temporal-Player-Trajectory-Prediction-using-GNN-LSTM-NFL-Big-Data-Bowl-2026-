...
- `test_input.csv`: An optional unlabelled test set for generating final leaderboard submissions.

**Note on `test_input.csv`**: This file is **not required for training**. It is used specifically for the `submission_script.py` to produce coordinate predictions for competition entry. If this file was generated by another script, you can replace the path in `nfl_train.py` with any other tracking CSV you wish to perform inference on.

The data is processed using the `NFLBigDataDataset` class, which handles:
